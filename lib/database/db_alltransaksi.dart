...
class DbAlltransaksi {
  ...
  initDB() async {
    ...
    final path = join(documentsDirectory.path, 'ALLTRANSAKSIBARU13MAR.db');

    return await openDatabase(path, version: 1, onOpen: (db) {},
        onCreate: (Database db, int version) async {
      await db.execute('''
      CREATE TABLE alltransaksibaru13mar(
  ...
  createAlltransaksiBaru(ModelAlltransaksiBaru newAlltransaksiBaru) async {
    ...
    final res =
        await db.insert('alltransaksibaru13mar', newAlltransaksiBaru.toJson());
    print('insert to database alltransaksibaru13mar');
    return res;
  }
  ...
  Future<int> deleteAlltransaksiBaru() async {
    final db = await database;
    final res = await db.rawDelete('DELETE FROM alltransaksibaru13mar');
  ...
  Future<int> deleteAlltransaksiBaruByid(int? id) async {
    final db = await database;
    final res = await db.delete('alltransaksibaru13mar', whereArgs: [id]);
  ...
  Future<List<ModelAlltransaksiBaru>> getAll() async {
    final db = await database;
    final res = await db.rawQuery('SELECT * FROM alltransaksibaru13mar');
  ...
  Future<List<ModelAlltransaksiBaru>> getAlltransaksiBaru(jenis_id) async {
    ...
    final res = await db.rawQuery(
        'SELECT * FROM alltransaksibaru13mar WHERE jenisform_id=? and user_id=? ORDER BY invoices_number DESC',
  ...
  Future<List<ModelAlltransaksiBaru>> getAllHistoryBaru() async {
    final db = await database;
    final res = await db.rawQuery(
        'SELECT * FROM alltransaksibaru13mar WHERE user_id=?',
  ...
  Future<List<ModelAlltransaksiBaru>> getAlltransaksiNominalBaru(year) async {
    final db = await database;
    final res = await db.rawQuery(
        'SELECT * FROM alltransaksibaru13mar WHERE user_id=? and nett !=? and year =? and jenisform_id=?',
  ...
  Future<List<ModelAlltransaksiBaru>> getAlltransaksiNominalByMonthBaru(
    ...
    final db = await database;
    final res = await db.rawQuery(
        'SELECT * FROM alltransaksibaru13mar WHERE user_id=? and nett !=? and month =? and year =? and jenisform_id=?',
  ...
  Future<List<ModelAlltransaksiBaru>> getAllNominalTransaksiBaru(
    ...
    final db = await database;
    final res = await db.rawQuery(
        'SELECT * FROM alltransaksibaru13mar WHERE invoices_number=? and user_id=? and jenisform_id=?',
  ...
  Future<List<Map<String, Object?>>> getAllinvoicesnumberbaru(idtoko) async {
    ...
    return await db.rawQuery(
        'SELECT * FROM alltransaksibaru13mar WHERE user_id=? and customer_id=? ORDER BY invoices_number DESC',
  ...
  Future<List<ModelAlltransaksiBaru>> getAlltransaksiBysearchBaru(
    ...
    final db = await database;
    final res = await db.rawQuery(
        'SELECT * FROM alltransaksibaru13mar WHERE jenisform_id=? and user_id=? and invoices_number LIKE ? ORDER BY invoices_number DESC',
...
//get with search lot
  Future<List<ModelAlltransaksiBaru>> getAlltransaksiBylotBaru(name) async {
    name = '';
    final db = await database;
    final res = await db.query("alltransaksibaru13mar",
  ...
  // Metode lain untuk akses ke database...
}
